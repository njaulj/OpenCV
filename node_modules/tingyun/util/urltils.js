...
var url = require('url');
module.exports = {
  errorMatch : function errorMatch(config, code) {
    var codes = [];
    if (config.error_collector && config.error_collector.ignored_status_codes) {
      codes = config.error_collector.ignored_status_codes;
    }
    return (code >= 400 || code < 100) && codes.indexOf(code) === -1;
  },

  /**
   * Get back the pieces of the URL that TingYun cares about. Apply these
   * restrictions, in order:
   *
   * 1. Ensure that after parsing the URL, there's at least '/'
   * 2. Strip off session trackers after ';' (a TingYun convention)
